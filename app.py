...
# ==================== BLOCOS DAS ABAS ====================
 ...
 if aba == "Dashboard":
    st.subheader("📊 Dashboard Único - Gestão Executiva & Pessoal")
    ...
    if not df.empty:
        ...
        saldo_liquido = entradas - (saidas_cc + cartao_passivo)

        col1, col2, col3, col4, col5 = st.columns(5)
        col1.metric("🟢 Entradas", f"R$ {entradas:,.2f}")
        col2.metric("🔴 Saídas (C/C)", f"R$ {saidas_cc:,.2f}", delta_color="inverse")
        col3.metric("💳 Passivo Cartões", f"R$ {cartao_passivo:,.2f}", delta_color="inverse")
        ...
        # ==================== ALERTAS E SCORE FINANCEIRO ====================
        if comprometimento_renda > 50:
            st.warning("⚠️ Alerta: Comprometimento de renda acima de 50%! Risco de endividamento.")
        if burn_rate_val > 0:
            st.error("🔥 Atenção: Queima de caixa detectada.")

        score_financeiro = max(0, min(100, 100 - comprometimento_renda + net_savings_rate))
        st.metric("⭐ Score Financeiro", f"{score_financeiro:.0f}/100", delta="Saúde Financeira")
        ...
        if not df_filtrado.empty:
            ...
            with col_g1:
                st.markdown("#### 1️⃣ Income vs Expense vs Cartão")
                ...
                chart_ie = alt.Chart(df_melt_ie).mark_line(strokeWidth=3, point=True).encode(
                    ...
                    color=alt.Color('Métrica:N', scale=alt.Scale(domain=['Income       
