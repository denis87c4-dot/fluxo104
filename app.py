...
aba = st.sidebar.radio("Navegação", [
    "Dashboard", "Dashboard 2", "Dashboard 3", "Resumo Geral", "Projections & Charts", "Monthly Audit 2", "Monthly Audit",
...
if aba == "Dashboard":
    ...
    st.subheader("📊 Dashboard 3 - Painel Executivo Premium com Filtros Avançados")
    st.markdown("Visão corporativa com **múltiplos filtros**, KPIs estratégicos e tabelas comparativas inteligentes.")
    ...
    if not df.empty:
        ...
        with col_f1:
            status_sel = st.selectbox("📌 Status", ["Todos", "Efetivado", "Budget"])
        with col_f2:
            tipo_sel = st.multiselect("📂 Tipo de Lançamento", df["Tipo"].unique().tolist(), default=df["Tipo"].unique().tolist())
        with col_f3:
        ...
        with col_v1:
            valor_min = st.number_input("💵 Valor Mínimo", min_value=0.0, value=0.0, step=100.0)
        with col_v2:
            valor_max = st.number_input("💵 Valor Máximo", min_value=0.0, value=float(df["Valor"].max()), step=100.0)
        ...
        # ==================== KPIs ====================
        entradas = df_filtrado[df_filtrado["Tipo"] == "Receita"]["Valor"].sum()
        saidas = df_filtrado[(df_filtrado["Tipo"] == "Despesa") & (~df_filtrado["Conta"].isin(cartoes_nomes))]["Valor"].sum()
        ...
        col_kpi2.metric("🔴 Saídas (C/C)", f"R$ {saidas:,.2f}", delta_color="inverse")
        col_kpi3.metric("💳 Passivo Cartões", f"R$ {cartao_passivo:,.2f}", delta_color="inverse")
        col_kpi4.metric("🔵 Transferências", f"R$ {transferencias:,.2f}")
        col_kpi5.metric("💰 Saldo Líquido", f"R$ {saldo_liquido:,.2f}")
        ...
        st.dataframe(df_cat_sum, use_container_width=True)

        st.markdown("### 🏦 Consolidado por Conta")
        df_acc_sum = df_filtrado.groupby("Conta")["Valor"].sum().reset_index().sort_values("Valor", ascending=False)
        st.dataframe(df_acc_sum, use_container_width=True)

        st.markdown("### 📄 Detalhamento dos Lançamentos Filtrados")
        st.dataframe(df_filtrado[["Tipo","Descricao","Categoria","Conta","Valor","Data","Status"]], use_container_width=True)

        # ==================== INDICADORES AVANÇADOS ====================
        st.markdown("### 📌 Indicadores Executivos")
        net_savings_rate = (saldo_liquido / entradas * 100) if entradas > 0 else 0.0
        comprometimento_renda = (saidas / entradas * 100) if entradas > 0 else 0.0
