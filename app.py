...
aba = st.sidebar.radio("Navegação", [
    "Dashboard", "Dashboard 2", "Transactions Details", "Resumo Geral", "Projections & Charts", "Monthly Audit 2", "Monthly Audit",
...
if aba == "Dashboard":
    ...
    if not df.empty:
        ...
        if not df_cat_sim.empty:
            df_cat_sim["% do Limite"] = df_cat_sim.apply(
        ...
        # ==================== PROJEÇÃO AUTOMÁTICA ====================
        st.markdown("### 📈 Projeção até o Fim do Mês")
        dias_passados = datetime.today().day
