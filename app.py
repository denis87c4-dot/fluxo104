...
if aba == "Dashboard":
    ...
    df = st.session_state.get("lancamentos", pd.DataFrame())
    ...
    if not df.empty:
        ...
        cartoes_df = st.session_state.get("cartoes", pd.DataFrame())
        cartoes_nomes = cartoes_df["Nome"].tolist() if not cartoes_df.empty else []
    ...
    if not df.empty:
        ...
        df_hist = df.copy()
        cartoes_nomes_hist = cartoes_df["Nome"].tolist() if not cartoes_df.empty else []
        ...
        # ORDENAÇÃO CRONOLÓGICA PARA O CÁLCULO CORRETO DO ACUMULADO
        ...
        # INVERSÃO PARA EXIBIÇÃO NA TELA (O MÊS MAIS RECENTE NO TOPO)
        pivot_hist_exibicao = pivot_hist.sort_values("Mês", ascending=False).reset_index(drop=True)

        pivot_hist_fmt = pivot_hist_exibicao.copy()
        ...
        def aplicar_estilo(df_styled, subset=None):
            if hasattr(df_styled, "map"):
                return df_styled.map(colorir_negativos, subset=subset)
            return df_styled.applymap(colorir_negativos, subset=subset)

        st.dataframe(
            aplicar_estilo(pivot_hist_fmt.set_index("Mês").style, subset=["Cash Flow", "Acumulado"]),
