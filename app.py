...
if aba == "Dashboard":
    st.subheader("📊 Executive Dashboard — Smart Control Center")
    st.markdown(
        "Painel financeiro integrado com filtros inteligentes, "
        "Budget × Efetivado, análise de cartões e visão executiva."
    )

    df_original = st.session_state.lancamentos.copy()
    ...
    if not df_original.empty:
        ...
        df_original["Valor"] = pd.to_numeric(
            df_original["Valor"], errors="coerce"
        ).fillna(0.0)

        df_original["Data"] = pd.to_datetime(
            df_original["Data"], errors="coerce"
        )

        df_original = df_original.dropna(subset=["Data"])

        cartoes_nomes = (
            st.session_state.cartoes["Nome"].astype(str).tolist()
        ...
        with st.expander("⚙️ Abrir / Fechar Filtros Avançados", expanded=True):

            # --------------------------------------------------------
            # LINHA 1 — PERÍODO E VISÃO
            # --------------------------------------------------------
            ...
            with f3:
                visao = st.selectbox(
                    ...
                    [
                        ...
                        "Efetivado",
                        "Budget",
                        "Comparativo Budget × Efetivado"
                    ],
                    key="dash_visao"
                )

            # --------------------------------------------------------
            # LINHA 2 — TIPO / CATEGORIA / CONTA
            # --------------------------------------------------------

            f4, f5, f6 = st.columns([1.5, 2.5, 2.5])

            with f4:
                tipos_disponiveis = sorted(
                    df_original["Tipo"].dropna().astype(str).unique().tolist()
                )
            ...
            # --------------------------------------------------------
            # LINHA 3 — ORIGEM / BUSCA / VALOR
            # --------------------------------------------------------

            f7, f8, f9 = st.columns([1.5, 3, 2])
            ...
            with f8:
                busca = st.text_input(
                    "🔎 Smart Search",
                    placeholder="Descrição, categoria, conta ou parcela...",
                    key="dash_busca"
                )

            valor_maximo = float(
                df_original["Valor"].max()
            ) if not df_original.empty else 0.0

            with f9:
                valor_range = st.slider(
                    "💵 Faixa de Valor",
                    min_value=0.0,
                    max_value=max(valor_maximo, 1.0),
                    value=(0.0, max(valor_maximo, 1.0)),
                    step=max(max(valor_maximo, 1.0) / 100, 1.0),
                    key="dash_valor"
                )
            ...
            with fc1:
                somente_despesas = st.checkbox(
                    "🔴 Somente despesas",
                    value=False,
                    key="dash_somente_despesas"
                )

            with fc2:
                excluir_transferencias = st.checkbox(
                    "🚫 Excluir transferências",
                    value=False,
                    key="dash_excluir_transferencias"
                )

            with fc3:
                somente_parcelados = st.checkbox(
                    "🔢 Somente parcelados",
                    value=False,
                    key="dash_somente_parcelados"
                )

        # ============================================================
        # APLICAÇÃO CENTRAL DOS FILTROS
        # ============================================================

        df_filtrado = df_original.copy()

        # Período
        data_inicio_dt = pd.to_datetime(periodo_inicio)
        data_fim_dt = pd.to_datetime(periodo_fim) + pd.Timedelta(days=1)
        ...
        df_filtrado = df_filtrado[
            (df_filtrado["Data"] >= data_inicio_dt) &
            (df_filtrado["Data"] < data_fim_dt)
        ]
        ...
        if categorias_filtro:
            df_filtrado = df_filtrado[
                df_filtrado["Categoria"].astype(str).isin(categorias_filtro)
            ]
        else:
        ...
        if contas_filtro:
            df_filtrado = df_filtrado[
                df_filtrado["Conta"].astype(str).isin(contas_filtro)
            ]
        else:
            df_filtrado = df_filtrado.iloc[0:0]

        # Origem
        if origem_filtro == "Cartão":
            df_filtrado = df_filtrado[
                df_filtrado["Conta"].isin(cartoes_nomes)
            ]

        elif origem_filtro == "Conta Corrente":
            df_filtrado = df_filtrado[
                ~df_filtrado["Conta"].isin(cartoes_nomes)
            ]
        ...
        # Faixa de valor
        df_filtrado = df_filtrado[
            (df_filtrado["Valor"] >= valor_range[0]) &
            (df_filtrado["Valor"] <= valor_range[1])
        ]
        ...
        # Excluir transferências
        if excluir_transferencias:
            df_filtrado = df_filtrado[
                df_filtrado["Tipo"] != "Transferência"
            ]

        # Somente parcelados
        if somente_parcelados:
            df_filtrado = df_filtrado[
                df_filtrado["Parcela"].astype(str).str.contains(
                    "/", na=False
                )
                &
                ~df_filtrado["Parcela"].astype(str).str.match(
                    r"^1/1$"
                )
            ]
        ...
        # Para visão comparativa, preservamos Budget e Efetivado.
        df_comparativo = df_filtrado.copy()

        df_efetivado = df_comparativo[
            df_comparativo["Status"] == "Efetivado"
        ].copy()

        df_budget = df_comparativo[
            df_comparativo["Status"] == "Budget"
        ].copy()

        # Para visão simples
        if visao == "Efetivado":
            df_analise = df_efetivado.copy()

        elif visao == "Budget":
            df_analise = df_budget.copy()

        else:
            df_analise = df_comparativo.copy()

        # ============================================================
        # FUNÇÕES AUXILIARES
        # ============================================================

        def soma(df_base, tipo=None, status=None, cartao=None):
            ...
            if cartao is True:
                temp = temp[temp["Conta"].isin(cartoes_nomes)]

            elif cartao is False:
                temp = temp[~temp["Conta"].isin(cartoes_nomes)]

            return float(temp["Valor"].sum())

        # ============================================================
        # KPIs PRINCIPAIS
        # ============================================================

        receitas_realizadas = soma(
            df_efetivado,
            tipo="Receita"
        )

        despesas_realizadas = soma(
            df_efetivado,
            tipo="Despesa",
            cartao=False
        )

        despesas_cartao_realizadas = soma(
            df_efetivado,
            tipo="Despesa",
            cartao=True
        )

        receitas_budget = soma(
            df_budget,
            tipo="Receita"
        )

        despesas_budget = soma(
            df_budget,
            tipo="Despesa",
            cartao=False
        )

        despesas_cartao_budget = soma(
            df_budget,
            tipo="Despesa",
            cartao=True
        )
        ...
        saldo_real = receitas_realizadas - despesas_realizadas

        # ============================================================
        # VARIAÇÕES BUDGET × EFETIVADO
        # ============================================================

        variacao_despesa = despesas_realizadas - despesas_budget

        percentual_budget_despesa = (
            despesas_realizadas / despesas_budget * 100
            if despesas_budget > 0
            else 0.0
        )

        saldo_budget = despesas_budget - despesas_realizadas

        taxa_poupanca = (
            saldo_real / receitas_realizadas * 100
            if receitas_realizadas > 0
            else 0.0
        )

        comprometimento = (
            despesas_realizadas / receitas_realizadas * 100
            if receitas_realizadas > 0
            else 0.0
        )

        cash_ratio = (
            receitas_realizadas / despesas_realizadas
        ...
        cinfo3.metric(
            "📅 Período",
            f"{periodo_inicio.strftime('%d/%m/%Y')} → "
            f"{periodo_fim.strftime('%d/%m/%Y')}"
        )

        cinfo4.metric(
            "💵 Volume Filtrado",
            f"R$ {df_analise['Valor'].sum():,.2f}"
        )
        ...
        with k1:
            st.metric(
                "📈 RECEITAS EFETIVADAS",
                f"R$ {receitas_realizadas:,.2f}",
                delta=(
                    f"Budget: R$ {receitas_budget:,.2f}"
                    if receitas_budget > 0
                    else "Sem Budget"
                )
            )
        ...
        with k2:
            st.metric(
                "📉 DESPESAS EFETIVADAS",
                f"R$ {despesas_realizadas:,.2f}",
                delta=(
                    f"{variacao_despesa:+,.2f} vs Budget"
                    if despesas_budget > 0
                    else "Sem Budget"
                ),
                delta_color="inverse"
        ...
        with k3:
            st.metric(
                "💳 PASSIVO CARTÕES",
                f"R$ {despesas_cartao_realizadas:,.2f}",
                delta=(
                    f"Budget: R$ {despesas_cartao_budget:,.2f}"
                    if despesas_cartao_budget > 0
                    else "Cartões"
                ),
                delta_color="inverse"
            )
        ...
        b3.metric(
            ...
            delta_color="normal" if saldo_budget >= 0 else "inverse"
        )

        b4.metric(
            "📊 EXECUÇÃO DO BUDGET",
            f"{percentual_budget_despesa:.1f}%"
        )

        # Barra visual do orçamento
        progresso_budget = min(
            max(percentual_budget_despesa / 100, 0),
            1
        )

        st.progress(
            progresso_budget,
            text=f"Execução do orçamento: {percentual_budget_despesa:.1f}%"
        )

        if percentual_budget_despesa > 100:
            st.error(
                f"🚨 ATENÇÃO: despesas efetivadas ultrapassaram o Budget "
                f"em R$ {abs(saldo_budget):,.2f}."
            )

        elif percentual_budget_despesa >= 80:
            st.warning(
                f"⚠️ Atenção: {percentual_budget_despesa:.1f}% "
                f"do orçamento já foi consumido."
            )

        else:
            st.success(
                f"✅ Orçamento sob controle: "
                f"{percentual_budget_despesa:.1f}% utilizado."
            )
        ...
        st.markdown("---")
        st.markdown("## 🏷️ Category Intelligence")

        if not df_comparativo.empty:

            cat_budget = (
                df_budget[
                    df_budget["Tipo"] == "Despesa"
                ]
            ...
            for cat in categorias:
                ...
                budget_val = float(cat_budget.get(cat, 0))
                real_val = float(cat_real.get(cat, 0))

                diferenca = budget_val - real_val

                percentual = (
                    real_val / budget_val * 100
                    if budget_val > 0
                    else 0
                )

                if budget_val == 0 and real_val > 0:
                    situacao = "⚠️ SEM BUDGET"
                ...
                dados_cat.append({
                    ...
                    "Budget": budget_val,
                    "Efetivado": real_val,
                    "Saldo Budget": diferenca,
                    "% Consumido": percentual,
                    "Situação": situacao
                })

            df_categoria = pd.DataFrame(dados_cat)
            ...
            if not df_categoria.empty:
                ...
                df_cat_display = df_categoria.copy()

                df_cat_display["Budget"] = (
                    df_cat_display["Budget"]
                    .apply(lambda x: f"R$ {x:,.2f}")
                )

                df_cat_display["Efetivado"] = (
                    df_cat_display["Efetivado"]
                    .apply(lambda x: f"R$ {x:,.2f}")
                )

                df_cat_display["Saldo Budget"] = (
                    df_cat_display["Saldo Budget"]
                    .apply(lambda x: f"R$ {x:,.2f}")
                )

                df_cat_display["% Consumido"] = (
                    df_cat_display["% Consumido"]
                ...
                estouradas = df_categoria[
                    df_categoria["% Consumido"] > 100
                ]

                sem_budget = df_categoria[
                    (df_categoria["Budget"] == 0) &
                    (df_categoria["Efetivado"] > 0)
                ]
                ...
                if not estouradas.empty:
                    st.error(
                        ...
                        + ", ".join(
                            estouradas["Categoria"].astype(str).tolist()
                        )
                    )

                if not sem_budget.empty:
                    st.warning(
                        "⚠️ Gastos realizados sem Budget: "
                        + ", ".join(
                            sem_budget["Categoria"].astype(str).tolist()
                        )
                    )
        ...
        st.markdown("---")
        st.markdown("## 📈 Financial Evolution")

        if not df_analise.empty:

            df_graf = df_analise.copy()
            ...
            despesas_g = (
                df_graf[
                    (df_graf["Tipo"] == "Despesa") &
                    (~df_graf["Conta"].isin(cartoes_nomes))
                ]
                .groupby("AnoMes")["Valor"]
                .sum()
            )
            ...
            meses_graf = sorted(
                set(receitas_g.index) |
                set(despesas_g.index) |
                set(cartoes_g.index)
            )
            ...
            df_evolucao = pd.DataFrame({
                "Mês": meses_graf,
                "Receita": [
                    receitas_g.get(m, 0.0)
                    for m in meses_graf
                ],
                "Despesa": [
                    despesas_g.get(m, 0.0)
                    for m in meses_graf
                ],
                "Cartão": [
                    cartoes_g.get(m, 0.0)
            ...
            df_evolucao["Cash Flow"] = (
                df_evolucao["Receita"] -
                df_evolucao["Despesa"]
            )

            df_evolucao["Acumulado"] = (
                df_evolucao["Cash Flow"].cumsum()
            )

            df_melt = df_evolucao.melt(
                id_vars="Mês",
                value_vars=[
                    "Receita",
                    "Despesa",
        ...
        if not df_analise.empty and "df_evolucao" in locals():

            df_cf = df_evolucao.melt(
                id_vars="Mês",
                value_vars=[
                    "Cash Flow",
                    "Acumulado"
        ...
        st.markdown("---")
        st.markdown("## 🔥 Top 10 Despesas")

        df_top = df_analise[
            df_analise["Tipo"] == "Despesa"
        ].copy()
        ...
        if not df_top.empty:
            ...
            df_top["Valor"] = df_top["Valor"].apply(
                lambda x: f"R$ {x:,.2f}"
            )
        ...
        if not df_analise.empty:
            ...
            df_final["Data"] = df_final["Data"].dt.strftime(
                "%d/%m/%Y"
            )
        ...
        st.info(
            "Nenhum lançamento cadastrado no sistema."
        )
