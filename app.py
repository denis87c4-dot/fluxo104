...
st.markdown(
    "Acompanhamento financeiro corporativo em tempo real com salvamento automático e inteligência preditiva."
)
...
arquivo_upload = st.sidebar.file_uploader(
    "Envie seu arquivo ZIP de backup", type="zip"
)
...
if arquivo_upload is not None:
  try:
    with zipfile.ZipFile(arquivo_upload, "r") as zip_ref:
      zip_ref.extractall(".")
    st.sidebar.success(
        "✅ Dados restaurados com sucesso! Recarregue a página."
    )
...
if aba == "Dashboard":
  ...
  if not df.empty:
    ...
    df_mes_ant = df[
        (df["Data"].dt.year == dt_ant.year) & (df["Data"].dt.month == dt_ant.month)
    ]
  ...
  if not df.empty:
    ...
    status_selecionado = st.selectbox(
        "📌 Filtrar por Status", status_opcoes, index=0
    )
    ...
    df_hist["Expense_Card"] = df_hist.apply(
        lambda r: r["Valor"]
        if r["Tipo"] == "Despesa" and r["Conta"] in cartoes_nomes_hist
        else 0.0,
        axis=1,
    )
      ...
      chart_ie = (
          ...
          .encode(
              ...
              color=alt.Color(
                  "Métrica:N",
                  scale=alt.Scale(
                      domain=["Income", "Expense (C/C)", "Passivo Cartão"],
  ...
  st.markdown(
      "Consolidação de Entradas, Saídas, Transferências e Cartões com filtro"
      " dinâmico de Status."
  )
  ...
  if not df.empty:
    ...
    total_saidas_cc = df_mes[
        (df_mes["Tipo"] == "Despesa")
        & (~df_mes["Conta"].isin(cartoes_nomes))
    ]["Valor"].sum()
    total_passivo_cartao = df_mes[
        (df_mes["Tipo"] == "Despesa") & (df_mes["Conta"].isin(cartoes_nomes))
    ]["Valor"].sum()
    total_transferencias = df_mes[df_mes["Tipo"] == "Transferência"][
        "Valor"
    ].sum()
    ...
    with col_rg1:
      st.metric(
          "🟢 Entradas", f"R$ {total_entradas:,.2f}", delta=status_selecionado
      )
    with col_rg2:
      st.metric(
          "🔴 Saídas (C/C)",
          f"R$ {total_saidas_cc:,.2f}",
          delta=status_selecionado,
          delta_color="inverse",
      )
    with col_rg3:
      st.metric(
    ...
    net_savings_rate = (
        (saldo_liquido / total_entradas * 100) if total_entradas > 0 else 0.0
    )
    comprometimento_renda = (
        (total_saidas_cc / total_entradas * 100) if total_entradas > 0 else 0.0
    )
    cash_ratio_val = (
        (total_entradas / total_saidas_cc) if total_saidas_cc > 0 else 0.0
    )
    ...
    with col_n3:
      st.metric(
          "🛡️ Cash Ratio", f"{cash_ratio_val:.2f}x", delta="Liquidez"
      )
    with col_n4:
      st.metric(
          "🔥 Burn Rate",
    ...
    st.markdown("---")
    st.markdown(
        f"### 📄 Detalhamento dos Lançamentos ({mes_selecionado_rg} -"
        f" {status_selecionado})"
    )
    if not df_mes.empty:
      df_exibicao = df_mes[
          [
      ...
      st.info(
          f"Nenhum lançamento encontrado para {mes_selecionado_rg} com status"
          f" {status_selecionado}."
      )
  else:
  ...
  if not df.empty:
    ...
    if len(pivot_graf) >= 2:
      ...
      for step in range(1, 7):
        ...
        dados_banda.append(
            {"Periodo": fut_str, "Valor": proj_central, "Tipo": "Projeção Esperada"}
        )
        dados_banda.append({
            "Periodo": fut_str,
    ...
    pivot_graf["Variacao_Despesa"] = (
        pivot_graf["Despesa"].diff().fillna(0.0)
    )
    chart_burn = (
          ...
          df[
              ...
              & (
                  df["Categoria"].str.contains(
                      "Moradia|Aluguel|Fixa|Conta|Dívida", case=False, na=False
                  )
              )
          ]
    ...
    line_patr = (
        alt.Chart(df_indep)
        .mark_line(strokeWidth=3, color="#2a9d8f")
        .encode(x="Mes:N", y=alt.Y("Patrimonio:Q", title="Capital (R$)"))
    )
    line_meta = (
        alt.Chart(df_indep)
        .mark_line(strokeWidth=2, strokeDash=[5, 5], color="#e76f51")
    ...
    st.metric(
        ...
        delta=(
            f"R$ {renda_passiva_estimada:,.2f} / mês de renda passiva teórica"
        ),
    )
...
elif aba == "Novo Dashboard Inteligente":
  ...
  df["AnoQuadrimestre"] = (
      ((df["Data"].dt.month - 1) // 4 + 1).astype(str)
      + "/"
  ...
  with col_f1:
    filtro_periodo = st.radio(
        "📅 Período", ["Mensal", "Trimestral", "Quadrimestral"]
    )
  with col_f2:
  ...
  with col_g1:
    ...
    chart_rd = (
        alt.Chart(pivot).mark_line(point=True).encode(x=periodo_col, y="Valor:Q", color="Tipo:N")
    )
  ...
  with col_g2:
    ...
    chart_cf = (
        alt.Chart(
            pivot.melt(
                id_vars=periodo_col, value_vars=["CashFlow", "Acumulado"]
            )
        )
        .mark_line(point=True)
        .encode(x=periodo_col, y="value:Q", color="variable:N")
    )
  ...
  heatmap = (
      alt.Chart(df_filtrado[df_filtrado["Tipo"] == "Despesa"])
      .mark_rect()
      .encode(
  ...
  df_abc["Acumulado_%"] = (
      df_abc["Valor"].cumsum() / df_abc["Valor"].sum() * 100
  )
  chart_abc = (
      alt.Chart(df_abc)
      .mark_bar()
      .encode(x="Categoria", y="Valor", tooltip=["Categoria", "Valor", "Acumulado_%"])
  )
  line_abc = (
      alt.Chart(df_abc)
      .mark_line(color="red", point=True)
      .encode(x="Categoria", y="Acumulado_%")
  )
  st.altair_chart(
      alt.layer(chart_abc, line_abc).resolve_scale(y="independent"),
      use_container_width=True,
  )
  ...
  indicadores = {
      "Taxa Poupança": (
          (pivot["CashFlow"].sum() / pivot["Receita"].sum() * 100)
          if pivot["Receita"].sum() > 0
          else 0
      ),
      "Comprom. Renda": (
          (pivot["Despesa"].sum() / pivot["Receita"].sum() * 100)
          if pivot["Receita"].sum() > 0
          else 0
      ),
      "Cash Ratio": (
          (pivot["Receita"].sum() / pivot["Despesa"].sum())
          if pivot["Despesa"].sum() > 0
          else 0
      ),
      "Burn Rate": (
          abs(pivot["CashFlow"].mean())
          if pivot["CashFlow"].mean() < 0
          else 0
      ),
  }
  if indicadores["Taxa Poupança"] < 10:
    st.warning(
        "⚠️ Sua taxa de poupança está abaixo de 10%. Considere reduzir despesas."
    )
  ...
  st.markdown(
      "Auditoria avançada de desempenho orçamentário com seleção de período,"
      " filtros, percentuais de desvio e análise de peso por categoria."
  )
  ...
  if not df.empty:
    ...
    with col_m1:
      mes_audit_selecionado = st.selectbox(
          "📅 Período de Auditoria", meses_disponiveis, key="audit_mes_sel"
      )

    ano_aud, mes_aud = map(int, mes_audit_selecionado.split("-"))

    df_mes_audit = df[
        (df["Data"].dt.year == ano_aud)
        & (df["Data"].dt.month == mes_aud)
        & (df["Tipo"] == "Despesa")
    ]
    ...
    if not df_mes_audit.empty:
      with col_m2:
        categorias_disponiveis = sorted(
            df_mes_audit["Categoria"].unique().tolist()
        )
        filtro_cat_audit = st.multiselect(
            "Filtrar Categorias",
            categorias_disponiveis,
            default=categorias_disponiveis,
        )
      with col_m3:
        filtro_busca_audit = st.text_input(
            "🔍 Buscar na Auditoria", placeholder="Descrição..."
        )

      if filtro_cat_audit:
        df_mes_audit = df_mes_audit[
            df_mes_audit["Categoria"].isin(filtro_cat_audit)
        ]
      if filtro_busca_audit.strip() != "":
        df_mes_audit = df_mes_audit[
            df_mes_audit["Descricao"].str.contains(
                filtro_busca_audit, case=False, na=False
            )
        ]

      df_budget_m = (
          df_mes_audit[df_mes_audit["Status"] == "Budget"]
          .groupby("Categoria")["Valor"]
          .sum()
      )
      df_entry_m = (
          df_mes_audit[df_mes_audit["Status"] == "Efetivado"]
          .groupby("Categoria")["Valor"]
          .sum()
      )

      todas_cats = sorted(
          list(set(df_budget_m.index.tolist() + df_entry_m.index.tolist()))
      )
      ...
      for cat in todas_cats:
        ...
        perc_consumido = (
            (e_val / b_val * 100)
            if b_val > 0
            else (100.0 if e_val > 0 else 0.0)
        )
        perc_do_total = (
            (e_val / total_geral_realizado * 100)
            if total_geral_realizado > 0
            else 0.0
        )
      ...
      ac2.metric(
          "💳 Total Realizado",
          f"R$ {tot_e:,.2f}",
          delta=f"{( (tot_e-tot_b)/tot_b*100 if tot_b>0 else 0 ):+.1f}% vs Budget",
          delta_color="inverse",
      )
      ac3.metric(
          "🎯 Saldo Orçamentário",
      ...
      st.markdown("---")
      st.markdown(
          f"### 📋 Tabela Analítica Consolidada por Categoria"
          f" ({mes_audit_selecionado})"
      )

      df_audit_fmt = df_audit_res.copy()
      df_audit_fmt["Budget"] = df_audit_fmt["Budget"].apply(
          lambda x: f"R$ {x:,.2f}"
      )
      df_audit_fmt["Realizado (Entry)"] = df_audit_fmt["Realizado (Entry)"].apply(
          lambda x: f"R$ {x:,.2f}"
      )
      df_audit_fmt["Diferença (R$)"] = df_audit_fmt["Diferença (R$)"].apply(
          lambda x: f"R$ {x:,.2f}"
      )

      st.dataframe(
          aplicar_estilo_tabela(
              df_audit_fmt.style, subset=["Diferença (R$)"]
          ),
          use_container_width=True,
      )
      ...
      cat_selecionada_drill = st.selectbox(
          "Escolha uma categoria para abrir o detalhamento transacional:",
          todas_cats,
          key="drill_cat",
      )

      if cat_selecionada_drill:
        df_drill = df_mes_audit[
            df_mes_audit["Categoria"] == cat_selecionada_drill
        ]
        if not df_drill.empty:
          df_drill_show = df_drill[
              ["Status", "Descricao", "Conta", "Valor", "Data", "Parcela"]
          ].copy()
          df_drill_show["Valor"] = df_drill_show["Valor"].apply(
              lambda x: f"R$ {x:,.2f}"
          )
          st.dataframe(df_drill_show, use_container_width=True)
        else:
          st.info(
              f"Nenhum lançamento encontrado para a categoria"
              f" {cat_selecionada_drill} no período."
          )
    else:
  ...
  st.markdown(
      "Indicadores financeiros avançados com filtros de Tipo, Status e Período"
  ...
  if not df.empty:
    ...
    tipo_selecionado = st.selectbox(
        "📌 Filtrar por Tipo", tipo_opcoes, index=0
    )
    ...
    periodo_selecionado = st.selectbox(
        "📅 Selecionar Período", periodo_opcoes, index=0
    )
    ...
    if periodo_selecionado == "Mensal":
      ...
      df["Periodo"] = (
          df["Data"].dt.year.astype(str) + "-Qd" + df["Quadrimestre"]
      )
    ...
    df["Expense_CC"] = df.apply(
        lambda r: r["Valor"]
        if r["Tipo"] == "Despesa" and r["Conta"] not in cartoes_nomes
        else 0.0,
        axis=1,
    )
    df["Expense_Card"] = df.apply(
        lambda r: r["Valor"]
        if r["Tipo"] == "Despesa" and r["Conta"] in cartoes_nomes
        else 0.0,
        axis=1,
    )
    df["Income_Val"] = df.apply(
    ...
    pivot_fin["Cash Flow"] = (
        pivot_fin["Receita"] - pivot_fin["Despesa (C/C)"]
    )
    pivot_fin["Acumulado"] = pivot_fin["Cash Flow"].cumsum()

    pivot_fin["Taxa Poupança (%)"] = (
        (
            (pivot_fin["Receita"] - pivot_fin["Despesa (C/C)"])
            / pivot_fin["Receita"]
            * 100
    ...
    pivot_fin["Comp. Renda (%)"] = (
        (pivot_fin["Despesa (C/C)"] / pivot_fin["Receita"] * 100)
        .replace([np.inf, -np.inf], 0)
        .fillna(0)
    )
    pivot_fin["Burn Rate"] = pivot_fin["Cash Flow"].apply(
        lambda x: abs(x) if x < 0 else 0
    )
    pivot_fin["Margem (%)"] = (
        (pivot_fin["Cash Flow"] / pivot_fin["Receita"] * 100)
        .replace([np.inf, -np.inf], 0)
        .fillna(0)
    )
    ...
    st.dataframe(
        aplicar_estilo_tabela(
            pivot_fmt.set_index("Periodo").style,
            subset=["Cash Flow", "Acumulado"],
        ),
        use_container_width=True,
    )
    ...
    df_melt_ie = pivot_fin.melt(
        id_vars="Periodo",
        value_vars=["Receita", "Despesa (C/C)", "Passivo Cartão"],
        var_name="Métrica",
        value_name="Valor",
    )
    chart_ie = (
        ...
        .encode(
            ...
            color=alt.Color(
                "Métrica:N",
                scale=alt.Scale(
                    domain=["Receita", "Despesa (C/C)", "Passivo Cartão"],
                    range=["#2a9d8f", "#e76f51", "#f4a261"],
                ),
            ),
    ...
    chart_cr = (
        alt.Chart(pivot_fin)
        .mark_line(point=True, strokeWidth=3, color="#e76f51")
        .encode(
            x=alt.X("Periodo:N", title="Período"),
    ...
    chart_margin = (
        alt.Chart(pivot_fin)
        .mark_line(point=True, strokeWidth=3, color="#f4a261")
        .encode(
            x=alt.X("Periodo:N", title="Período"),
  ...
  if not df.empty:
    ...
    mes_selecionado_si2 = st.selectbox(
        "📅 Selecione o Mês (Ano-Mês)", meses_disponiveis, key="sel_mes_si2"
    )
    ...
    df_range = pivot_stats2.melt(
        id_vars="Mês",
        value_vars=["Mínimo", "Máximo"],
        var_name="Faixa",
        value_name="Valor",
    )
    chart_range = (
        ...
        .encode(
            ...
            color=alt.Color(
                "Faixa:N",
                scale=alt.Scale(
                    domain=["Mínimo", "Máximo"], range=["#264653", "#e76f51"]
                ),
            ),
  ...
  st.markdown(
      "Indicadores estatísticos avançados com filtros de Tipo, Status e Período"
      " (Mensal, Trimestral, Quadrimestral)."
  )
  ...
  if not df.empty:
    ...
    pivot_stats = pivot_stats.rename(
        columns={
            "Income_Val": "Receita",
            "Expense_CC": "Despesa (C/C)",
            "Expense_Card": "Passivo Cartão",
    ...
    pivot_stats["Cash Flow"] = (
        pivot_stats["Receita"] - pivot_stats["Despesa (C/C)"]
    )
    pivot_stats["Acumulado"] = pivot_stats["Cash Flow"].cumsum()

    pivot_stats["Taxa Poupança (%)"] = (
        (
            (pivot_stats["Receita"] - pivot_stats["Despesa (C/C)"])
            / pivot_stats["Receita"]
            * 100
    ...
    pivot_stats["Comp. Renda (%)"] = (
        (pivot_stats["Despesa (C/C)"] / pivot_stats["Receita"] * 100)
        .replace([np.inf, -np.inf], 0)
        .fillna(0)
    )
    ...
    for col in [
      ...
      pivot_fmt[col] = pivot_fmt[col].apply(lambda x: f"R$ {x:,.2f}")
    pivot_fmt["Taxa Poupança (%)"] = pivot_fmt["Taxa Poupança (%)"].apply(
        lambda x: f"{x:.1f}%"
    )
    pivot_fmt["Comp. Renda (%)"] = pivot_fmt["Comp. Renda (%)"].apply(
        lambda x: f"{x:.1f}%"
    )
    ...
    pivot_stats["Burn Rate"] = pivot_stats["Cash Flow"].apply(
        lambda x: abs(x) if x < 0 else 0
    )
    chart_burn = (
        alt.Chart(pivot_stats)
  ...
  if tipo == "Transferência":
    ...
    with col_t1:
      conta_saida_escolhida = st.selectbox(
          "Conta Saída (Origem)", contas_base + ["+ Incluir Novo Cartão/Conta..."]
      )
    ...
    conta_escolhida = st.selectbox(
        "Account (Conta / Cartão)", contas_base + ["+ Incluir Novo Cartão/Conta..."]
    )
  ...
  with st.form("form_nova_categoria"):
    nova_categoria = st.text_input(
        "Nova Categoria", placeholder="Ex: Investimentos, Educação..."
    )
