...
st.markdown(
    'Acompanhamento financeiro corporativo em tempo real com salvamento'
    ' automático e inteligência preditiva.'
)
...
if os.path.exists(ARQUIVO_CARTOES):
  ...
  st.session_state.cartoes = pd.DataFrame(
      columns=['Nome', 'Fechamento', 'Limite', 'Vencimento']
  )
...
if aba == 'Dashboard':
  ...
  if not df.empty:
    ...
    with col_f1:
      mes_selecionado = st.selectbox(
          '📅 Período de Análise (Mês/Ano)', meses_disponiveis, index=0
      )
    ...
    df_mes_ant = df[
        (df['Data'].dt.year == dt_ant.year)
        & (df['Data'].dt.month == dt_ant.month)
    ]

    receitas_mes = df_mes_atual[
        (df_mes_atual['Tipo'] == 'Receita')
        & (df_mes_atual['Status'] == 'Efetivado')
    ]['Valor'].sum()
    receitas_ant = df_mes_ant[
        (df_mes_ant['Tipo'] == 'Receita')
        & (df_mes_ant['Status'] == 'Efetivado')
    ]['Valor'].sum()
    delta_rec = (
        ((receitas_mes - receitas_ant) / receitas_ant * 100)
        if receitas_ant > 0
        else 0.0
    )
    ...
    delta_desp = (
        ((despesas_conta_corrente - despesas_ant_cc) / despesas_ant_cc * 100)
        if despesas_ant_cc > 0
        else 0.0
    )

    budget_despesas = df_mes_atual[
        (df_mes_atual['Tipo'] == 'Despesa')
        & (df_mes_atual['Status'] == 'Budget')
    ]['Valor'].sum()
    budget_receitas = df_mes_atual[
        (df_mes_atual['Tipo'] == 'Receita')
        & (df_mes_atual['Status'] == 'Budget')
    ]['Valor'].sum()
    ...
    if not df_vencidas.empty:
      ...
      texto_vencidas_detalhe = "<span style='color: #2a9d8f; font-weight: bold;'>R$ 0,00 (Nenhuma vencida)</span>"
    ...
    despesas_por_cartao = (
        df_cartoes_mes.groupby('Conta')['Valor'].sum()
        if not df_cartoes_mes.empty
        else pd.Series(dtype=float)
    )
    despesas_cartao_mes = df_cartoes_mes['Valor'].sum()

    net_savings_rate = (
        ((receitas_mes - despesas_conta_corrente) / receitas_mes * 100)
        if receitas_mes > 0
        else 0.0
    )
    comprometimento_renda = (
        (despesas_conta_corrente / receitas_mes * 100)
        if receitas_mes > 0
        else 0.0
    )

    contas_liquidas_mes = df_mes_atual[
        (df_mes_atual['Tipo'] == 'Receita')
        & (~df_mes_atual['Conta'].isin(cartoes_nomes))
    ]['Valor'].sum()
    cash_ratio_val = (
        (contas_liquidas_mes / despesas_conta_corrente)
        if despesas_conta_corrente > 0
        else 0.0
    )
    ...
    texto_vencidas_detalhe = (
        "<span style='color: #2a9d8f; font-weight: bold;'>R$ 0,00</span>"
    )
    delta_rec = 0.0
    delta_desp = 0.0
  ...
  with col1:
    st.metric(
        '📈 RECEITAS DO MÊS',
        f'R$ {receitas_mes:,.2f}',
        delta=f'{delta_rec:+.1f}% vs mês ant.',
    )
  with col2:
    st.metric(
        '📉 DESPESAS (C/C)',
        f'R$ {despesas_conta_corrente:,.2f}',
        delta=f'{delta_desp:+.1f}% vs mês ant.',
        delta_color='inverse',
    )
  with col3:
    st.metric(
        '💳 PASSIVO CARTÕES (Fatura)',
        f'R$ {despesas_cartao_mes:,.2f}',
        delta='Saldo Devedor',
        delta_color='inverse',
    )
    if not despesas_por_cartao.empty:
      for cartao, val in despesas_por_cartao.items():
        st.markdown(
            f"<small>• <b>{cartao}</b>: R$ {val:,.2f}</small>",
  ...
  with col_n1:
    st.metric(
        '💰 TAXA DE POUPANÇA',
        f'{net_savings_rate:.1f}%',
        delta='Net Savings Rate',
        delta_color='normal',
    )
  with col_n2:
    st.metric(
        '📊 COMPROP. DE RENDA',
        f'{comprometimento_renda:.1f}%',
        delta='Gastos / Receitas',
        delta_color='inverse',
    )
  with col_n3:
    st.metric(
        '🛡️ CASH RATIO',
        f'{cash_ratio_val:.2f}x',
        delta='Liquidez Imediata',
        delta_color='normal',
    )
  with col_n4:
    st.metric(
        '🔥 BURN RATE',
        f'R$ {burn_rate_val:,.2f}',
        delta='Queima de Caixa',
        delta_color='inverse',
    )
  ...
  st.markdown(
      'Tabela geral contemplando **Income**, **Expense (C/C)**, **Passivo'
      ' Cartões** e **Cash Flow** ordenados temporalmente.'
  )
  ...
  if not df.empty:
    ...
    df_hist['Expense_CC'] = df_hist.apply(
        lambda r: (
            r['Valor']
            if r['Tipo'] == 'Despesa' and r['Conta'] not in cartoes_nomes_hist
            else 0.0
        ),
        axis=1,
    )
    df_hist['Expense_Card'] = df_hist.apply(
        lambda r: (
            r['Valor']
            if r['Tipo'] == 'Despesa'
            and r['Conta'] in cartoes_nomes_hist
            and r['Status'] == 'Budget'
            else 0.0
        ),
        axis=1,
    ...
    st.dataframe(
        aplicar_estilo_tabela(
            pivot_hist_fmt.set_index('Mês').style,
            subset=['Cash Flow', 'Acumulado'],
        ),
    ...
    st.info(
        'Nenhum lançamento registrado para exibir a tabela consolidada e os'
        ' gráficos.'
    )
  ...
  st.markdown(
      'Visão consolidada e limpa das Entradas, Saídas e Transferências'
      ' estritamente **Efetivadas** no mês selecionado.'
  )
  ...
  if not df.empty:
    ...
    with col_sel1:
      mes_selecionado_rg = st.selectbox(
          '📅 Selecione o Mês (Ano-Mês)',
          meses_disponiveis,
          key='sel_mes_resumo_geral',
      )
    ...
    total_entradas = df_mes_efetivado[df_mes_efetivado['Tipo'] == 'Receita'][
        'Valor'
    ].sum()
    ...
    total_transferencias = df_mes_efetivado[
        df_mes_efetivado['Tipo'] == 'Transferência'
    ]['Valor'].sum()
    ...
    with col_rg1:
      st.metric(
          '🟢 Total Entradas', f'R$ {total_entradas:,.2f}', delta='Efetivado'
      )
    with col_rg2:
      st.metric(
          '🔴 Saídas (C/C)',
          f'R$ {total_saidas_cc:,.2f}',
          delta='Efetivado',
          delta_color='inverse',
      )
    with col_rg3:
    ...
    with col_rg4:
      st.metric(
          '🔵 Transferências',
          f'R$ {total_transferencias:,.2f}',
          delta='Efetivado',
      )
    with col_rg5:
      st.metric(
          '💰 Saldo Líquido',
          f'R$ {saldo_liquido_efetivado:,.2f}',
          delta='Caixa Real',
          delta_color='normal',
      )
    ...
    st.markdown('---')
    st.markdown(
        f'### 📄 Detalhamento dos Lançamentos Efetivados ({mes_selecionado_rg})'
    )

    if not df_mes_efetivado.empty:
      df_exibicao = df_mes_efetivado[
          [
              'Tipo',
              'Descricao',
              'Categoria',
      ...
      st.dataframe(
          aplicar_estilo_tabela(df_exibicao.style), use_container_width=True
      )
    else:
      st.info(
          f'Nenhum lançamento com status **Efetivado** encontrado para o período'
          f' de {mes_selecionado_rg}.'
      )
  else:
  ...
  st.subheader(
      '📈 Projections & Charts (12 Gráficos e Parâmetros Avançados de Elite)'
  )
  st.markdown(
      'Central completa contendo a **Célula Suspensa** e módulos analíticos de'
      ' projeção, risco e inteligência financeira.'
  )
  ...
  if not df.empty:
    ...
    df['Expense_CC'] = df.apply(
        lambda r: (
            r['Valor']
            if r['Tipo'] == 'Despesa' and r['Conta'] not in cartoes_nomes_proj
            else 0.0
        ),
        axis=1,
    )
    df['Income_Val'] = df.apply(
        lambda r: r['Valor'] if r['Tipo'] == 'Receita' else 0.0, axis=1
    )

    pivot_graf = df.pivot_table(
        index='AnoMes',
        values=['Income_Val', 'Expense_CC'],
        aggfunc='sum',
        fill_value=0.0,
    ).reset_index()
    pivot_graf = pivot_graf.rename(
        columns={'Income_Val': 'Receita', 'Expense_CC': 'Despesa'}
    )
    ...
    st.markdown('---')
    st.markdown(
        '### 🛸 Célula Suspensa (Simulador Preditivo de Ajuste de Orçamento)'
    )
    st.markdown(
        'Painel flutuante interativo para testar impactos imediatos no seu'
        ' fluxo de caixa futuro simulando cortes ou injeções de capital.'
    )

    with st.container():
      st.info(
          '💡 **Painel de Simulação Ativo**: Ajuste os parâmetros abaixo para'
          ' projetar o impacto direto no acumulado dos próximos meses.'
      )
      cs_col1, cs_col2, cs_col3 = st.columns(3)
      with cs_col1:
        fator_ajuste_despesa = st.slider(
            'Ajuste Percentual em Despesas (%)',
            min_value=-50,
            max_value=50,
            value=0,
            step=5,
      ...
      with cs_col2:
        aporte_extra_mensal = st.number_input(
            'Injeção / Retirada Fixa Mensal (R$)', value=0.0, step=100.0
        )
      with cs_col3:
        horizonte_simulacao = st.slider(
            'Horizonte de Projeção (Meses)', min_value=3, max_value=24, value=6
        )

      if len(pivot_graf) > 0:
        ultima_receita = pivot_graf['Receita'].iloc[-1]
        ultima_despesa = pivot_graf['Despesa'].iloc[-1]
        despesa_ajustada = ultima_despesa * (
            1 + (fator_ajuste_despesa / 100.0)
        )
        fluxo_simulado = (
            ultima_receita + aporte_extra_mensal
        ) - despesa_ajustada

        st.metric(
            'Projeção de Cash Flow Mensal Ajustado (Célula Suspensa)',
            f'R$ {fluxo_simulado:,.2f}',
            delta=f'{fator_ajuste_despesa:+d}% nas despesas',
        )
    st.markdown('---')

    st.markdown(
        '### 1️⃣ Projeção de Fôlego de Caixa (Runway Preditivo em Meses)'
    )
    saldo_atual_caixa = (
        pivot_graf['Acumulado'].iloc[-1] if not pivot_graf.empty else 0.0
    )
    media_despesas_recente = (
        pivot_graf['Despesa'].tail(3).mean()
        if len(pivot_graf) >= 3
        else pivot_graf['Despesa'].mean()
    )

    meses_proj_runway = np.arange(0, 13)
    if media_despesas_recente > 0:
      curva_runway = [
          max(0.0, saldo_atual_caixa - (media_despesas_recente * m))
          for m in meses_proj_runway
      ]
    else:
      curva_runway = [saldo_atual_caixa] * 13

    df_runway = pd.DataFrame({
        'Meses_Futuros': [f'Mês +{m}' for m in meses_proj_runway],
        'Saldo_Projetado': curva_runway,
    })
    ...
    chart_runway = (
        ...
        .mark_area(
            color='#1f77b4',
            opacity=0.5,
            line={'color': '#0d3b66', 'strokeWidth': 3},
        )
        .encode(
    ...
    st.markdown(
        '### 2️⃣ Projeção de Despesas com Banda de Confiança Estatística'
    )
    if len(pivot_graf) >= 2:
      ...
      ultimo_dt = datetime.strptime(
          pivot_graf['AnoMes'].max() + '-01', '%Y-%m-%d'
      )
      ...
      for step in range(1, 7):
        ...
        dados_banda.append({
            'Periodo': fut_str,
            'Valor': proj_central,
            'Tipo': 'Projeção Esperada',
        })
        dados_banda.append({
            'Periodo': fut_str,
            'Valor': proj_central + (1.96 * desvio_padrao_desp),
            'Tipo': 'Limite Superior (Pessimista)',
        })
        dados_banda.append({
            'Periodo': fut_str,
            'Valor': max(0.0, proj_central - (1.96 * desvio_padrao_desp)),
            'Tipo': 'Limite Inferior (Otimista)',
        })
              ...
              color=alt.Color(
                  ...
                  scale=alt.Scale(
                      domain=[
                          ...
                          'Limite Inferior (Otimista)',
                      ],
                      range=['#f4a261', '#e76f51', '#2a9d8f'],
                  ),
              ),
          )
          .properties(height=350)
    ...
    st.markdown(
        '### 3️⃣ Simulação de Atingimento de Meta de Patrimônio (Crescimento'
        ' Composto)'
    )
    taxa_juros_anual = st.slider(
        'Taxa de Retorno Anual Estimada dos Investimentos (% a.a.)',
        min_value=0.0,
        max_value=20.0,
        value=8.0,
        step=0.5,
    ...
    poupanca_media_mensal = (
        pivot_graf['CashFlow'].mean() if not pivot_graf.empty else 0.0
    )
    ...
    for m in range(1, 13):
      ...
      patr_acumulado = (patr_acumulado * (1 + taxa_mensal)) + (
          poupanca_media_mensal
      )
      dados_patrimonio.append(
          {'Mes': fut_dt.strftime('%Y-%m'), 'Patrimonio_Projetado': patr_acumulado}
      )
    ...
    st.markdown(
        '### 4️⃣ Gráfico de Tendência de Queima de Caixa (Burn Rate Velocity)'
    )
    pivot_graf['Variacao_Despesa'] = pivot_graf['Despesa'].diff().fillna(0.0)
    chart_burn = (
        alt.Chart(pivot_graf)
        .mark_bar()
        .encode(
    ...
    st.markdown(
        '### 5️⃣ Curva ABC de Gastos (Foco em Categorias de Maior Impacto)'
    )
    df_despesas_totais = (
        df[df['Tipo'] == 'Despesa']
    ...
    if not df_despesas_totais.empty:
      df_despesas_totais = df_despesas_totais.sort_values(
          by='Valor', ascending=False
      )
      df_despesas_totais['Acumulado_%'] = (
          df_despesas_totais['Valor'].cumsum()
          / df_despesas_totais['Valor'].sum()
      ) * 100

      base_abc = alt.Chart(df_despesas_totais).encode(
      ...
      line_abc = base_abc.mark_line(
          strokeWidth=3, color='#e76f51', point=True
      ).encode(
          y=alt.Y(
              'Acumulado_%:Q',
              title='Acumulado (%)',
    ...
    if 'Categoria' in df.columns:
      df_fixo = (
          df[
              (df['Tipo'] == 'Despesa')
              & (
                  df['Categoria'].str.contains(
                      'Moradia|Aluguel|Fixa|Conta|Dívida',
                      case=False,
                      na=False,
                  )
              )
      ...
      df_flex = (
          pivot_graf[['AnoMes', 'Receita']]
          .merge(df_fixo, on='AnoMes', how='left')
          .fillna(0.0)
      )
    ...
    st.markdown(
        '### 7️⃣ Projeção para Independência Financeira (Milestones)'
    )
    meta_patrimonio_indep = st.number_input(
        'Defina sua Meta de Patrimônio para Independência (R$)',
        value=500000.0,
        step=50000.0,
    )
    ...
    for m in range(1, 37):
      ...
      dados_indep.append({
          ...
          'Patrimonio': patr_indep,
          'Meta': meta_patrimonio_indep,
      })

    df_indep = pd.DataFrame(dados_indep)
    line_patr = alt.Chart(df_indep).mark_line(
        strokeWidth=3, color='#2a9d8f'
    ).encode(x='Mes:N', y=alt.Y('Patrimonio:Q', title='Capital (R$)'))
    line_meta = alt.Chart(df_indep).mark_line(
        strokeWidth=2, strokeDash=[5, 5], color='#e76f51'
    ).encode(x='Mes:N', y='Meta:Q')
    chart_milestone = (
        alt.layer(line_patr, line_meta).properties(height=350).interactive()
    )
    ...
    if not df_heatmap.empty:
      ...
      df_melted = pivot_heat.melt(
          id_vars='Categoria', var_name='Mes', value_name='Gasto'
      )
    ...
    st.markdown(
        '### 9️⃣ Simulação de Monte Carlo Pessoal (Stress Test de Volatilidade)'
    )
    st.markdown(
        'Simula 100 trajetórias estocásticas de caixa baseadas no desvio'
        ' padrão histórico para avaliar o risco de saldo negativo.'
    )
    if len(pivot_graf) >= 2:
      media_cf = pivot_graf['CashFlow'].mean()
      std_cf = (
          pivot_graf['CashFlow'].std() if len(pivot_graf) > 1 else 100.0
      )
      ...
      for sim in range(100):
        ...
        for m in range(1, 13):
          ...
          simulacoes_mc.append(
              {'Simulacao': sim, 'Mes': f'Mês +{m}', 'Saldo': saldo_sim}
          )

      df_mc = pd.DataFrame(simulacoes_mc)
      chart_mc = (
          alt.Chart(df_mc)
          .mark_line(opacity=0.2, color='#264653')
          .encode(
    ...
    st.markdown(
        '### 🔟 Índice de Resiliência de Fluxo de Caixa (Resilience Index)'
    )
    if len(pivot_graf) > 0:
      df_res = pivot_graf.copy()
      df_res['Resiliencia'] = np.clip(
          ((df_res['CashFlow'] / (df_res['Despesa'] + 1)) * 50) + 50, 0, 100
      )
      ...
      chart_res = (
          ...
          .encode(
              ...
              y=alt.Y(
                  'Resiliencia:Q',
                  title='Índice de Resiliência (0 a 100)',
    ...
    st.markdown(
        '### 1️⃣1️⃣ Dispersão de Despesas e Elasticidade (Valor vs Dia do Mês)'
    )
    ...
    st.markdown(
        '### 1️⃣2️⃣ Índice de Autonomia de Renda Passiva (Financial Freedom'
        ' Gauge)'
    )
    taxa_retorno_passivo = st.slider(
        'Taxa de Retorno Anual para Renda Passiva (% a.a.)',
        min_value=1.0,
        max_value=15.0,
        value=6.0,
        step=0.5,
    ...
    media_despesas_anual = (
        pivot_graf['Despesa'].mean() if not pivot_graf.empty else 1.0
    )
    renda_passiva_estimada = saldo_atual_caixa * taxa_mes_passivo
    autonomia_pct = min(
        (
            renda_passiva_estimada
            / (media_despesas_anual if media_despesas_anual > 0 else 1.0)
        )
        * 100,
        100.0,
    )
    ...
    st.metric(
        'Grau de Independência Atual',
        f'{autonomia_pct:.2f}% dos gastos cobertos',
        delta=f'R$ {renda_passiva_estimada:,.2f} / mês de renda passiva teórica',
    )
    st.progress(int(max(0, min(100, autonomia_pct))))

  else:
    st.info(
        'Nenhum lançamento registrado para exibir os gráficos analíticos e de'
        ' projeção.'
    )
  ...
  st.markdown(
      'Acompanhamento estrito do mês atual por categoria: **Budget** vs **Entry'
      ' (Efetivado/Realizado)** e a **Diferença**.'
  )
  ...
  if not df.empty:
    ...
    if not df_mes.empty:
      df_budget_mes = df_mes[df_mes['Status'] == 'Budget'].groupby(
          'Categoria'
      )['Valor'].sum()
      df_entry_mes = df_mes[df_mes['Status'] == 'Efetivado'].groupby(
          'Categoria'
      )['Valor'].sum()

      todas_categorias = sorted(
          list(set(df_budget_mes.index.tolist() + df_entry_mes.index.tolist()))
      )
      ...
      for cat in todas_categorias:
        ...
        if dif_val < 0:
          status_pagamento = (
              f'⚠️ Estourado/Pendente (R$ {abs(dif_val):,.2f} acima)'
          )
        else:
      ...
      df_audit_fmt['1. Budget (Mês)'] = df_audit_fmt['1. Budget (Mês)'].apply(
          lambda x: f'R$ {x:,.2f}'
      )
      df_audit_fmt['2. Entry (Efetivado)'] = df_audit_fmt[
          '2. Entry (Efetivado)'
      ].apply(lambda x: f'R$ {x:,.2f}')
      df_audit_fmt['3. Diferença (Budget - Entry)'] = df_audit_fmt[
          '3. Diferença (Budget - Entry)'
      ].apply(lambda x: f'R$ {x:,.2f}')

      st.dataframe(
          aplicar_estilo_tabela(
              df_audit_fmt.style, subset=['3. Diferença (Budget - Entry)']
          ),
          use_container_width=True,
      )
      ...
      hoje_dt = pd.to_datetime(datetime.today().date())
      vencidas_mes = df_mes[
          (df_mes['Status'] == 'Budget') & (df_mes['Data'] < hoje_dt)
      ]
      if not vencidas_mes.empty:
        st.markdown(
            '### ⚠️ Alerta de Contas Vencidas (Budget Pendente no Mês)'
        )
        df_venc_fmt = vencidas_mes[
            ['Data', 'Descricao', 'Categoria', 'Conta', 'Valor']
        ].copy()
        df_venc_fmt['Valor'] = df_venc_fmt['Valor'].apply(
            lambda x: f'R$ {x:,.2f}'
        )
  ...
  st.markdown(
      'Indicadores financeiros calculados mês a mês com base no planejamento'
      ' (Budget).'
  )
  ...
  if not df.empty and not df[df['Status'] == 'Budget'].empty:
    ...
    for m in meses:
      ...
      debts = df_m[
          (df_m['Tipo'] == 'Despesa')
          & (
              df_m['Categoria'].str.contains(
                  'debt|dívida', case=False, na=False
              )
          )
      ]['Valor'].sum()
      cartoes_nomes = (
      ...
      credit_card = df_m[
          (df_m['Tipo'] == 'Despesa')
          & (
              (df_m['Conta'].isin(cartoes_nomes))
              | (
                  df_m['Categoria'].str.contains(
                      'credit|cartão', case=False, na=False
                  )
              )
          )
      ]['Valor'].sum()
      ...
      burn_rate_str = (
          f'R$ {burn_rate:,.2f} (Queima)'
          if burn_rate > 0
          else 'R$ 0,00 (Sem Queima)'
      )

      df_ate_mes = df_b[df_b['AnoMes'] <= m]
      patrimonio_liquido_est = (
          df_ate_mes[df_ate_mes['Tipo'] == 'Receita']['Valor'].sum()
          - df_ate_mes[df_ate_mes['Tipo'] == 'Despesa']['Valor'].sum()
      )

      if burn_rate > 0:
        runway_meses = patrimonio_liquido_est / burn_rate
        runway_str = (
            f'{runway_meses:.1f} meses' if runway_meses > 0 else '0.0 meses'
        )
      else:
      ...
      if obrigacoes_mes > 0:
        ...
        dscr_final = (
            f'{dscr_val:.2f}x (Seguro > 1.2)'
            if dscr_val >= 1.2
            else f'{dscr_val:.2f}x (⚠️ Alerta < 1.2)'
        )
      else:
        dscr_final = 'N/A (Sem Dívidas/Cartão)'

      df_historico_ate_mes = df_b[
          (df_b['AnoMes'] <= m) & (df_b['Tipo'] == 'Despesa')
      ]
      ...
      if len(gastos_por_mes) > 1:
        ...
        cv_val = (
            (desv_hist / media_hist) * 100 if media_hist > 0 else 0.0
        )
    ...
    st.dataframe(
        pd.DataFrame(dados_avancados_3).set_index('Mês'),
        use_container_width=True,
    )
    ...
    for m in meses:
      ...
      cash_ratio_str = (
          f'{cash_ratio:.2f}x (Coberto)'
          if cash_ratio >= 1.0
          else f'{cash_ratio:.2f}x (⚠️ Ajuste Caixa)'
      )
      net_savings = (
    ...
    st.dataframe(
        pd.DataFrame(dados_avancados_4_6).set_index('Mês'),
        use_container_width=True,
    )
    ...
    for m in meses:
      ...
      juros_mes = df_m[
          ...
          & (
              df_m['Categoria'].str.contains(
                  'juros|interest|financiamento', case=False, na=False
              )
          )
      ]['Valor'].sum()
      if juros_mes > 0:
        ...
        interest_cov_str = (
            f'{interest_coverage:.2f}x (Seguro > 3.0)'
            if interest_coverage >= 3.0
            else f'{interest_coverage:.2f}x (⚠️ Alerta < 3.0)'
        )
      else:
      ...
      dti_str = (
          f'{dti_val:.1f}% (⚠️ Alto > 40%)'
          if dti_val > 40.0
          else f'{dti_val:.1f}% (Saudável <= 40%)'
      )
    ...
    st.dataframe(
        pd.DataFrame(dados_avancados_7_9).set_index('Mês'),
        use_container_width=True,
    )
  else:
    st.info(
        'Nenhum dado de Budget cadastrado para calcular os indicadores'
        ' financeiros.'
    )
  ...
  st.markdown(
      'Parâmetros estatísticos avançados, assimetria, curtose e detecção de'
      ' outliers.'
  )
  ...
  if not df.empty and not df[df['Status'] == 'Budget'].empty:
    ...
    pivot_mensal = df_b.pivot_table(
        index='AnoMes', columns='Tipo', values='Valor', aggfunc='sum', fill_value=0.0
    )
    ...
    pivot_mensal = pivot_mensal.rename(
        columns={'Receita': 'Income', 'Despesa': 'Expense'}
    )
    pivot_mensal['Cash Flow'] = (
        pivot_mensal['Income'] - pivot_mensal['Expense']
    )
    ...
    if len(pivot_mensal) > 2:
      ...
      desvio_padrao = (
          valores_exp.std() if len(valores_exp) > 1 else 0.0
      )

      n_obs = len(valores_exp)
      if desvio_padrao > 0 and n_obs > 2:
        skewness = (n_obs / ((n_obs - 1) * (n_obs - 2))) * np.sum(
            ((valores_exp - media_geral) / desvio_padrao) ** 3
        )
        kurtosis = (
            (n_obs * (n_obs + 1))
            / ((n_obs - 1) * (n_obs - 2) * (n_obs - 3))
        ) * np.sum(
            ((valores_exp - media_geral) / desvio_padrao) ** 4
        ) - (
            3
            * (n_obs - 1) ** 2
            / ((n_obs - 2) * (n_obs - 3))
      ...
      col_stat1.metric(
          '📐 Assimetria de Gastos (Skewness)',
          f'{skewness:.2f}',
          delta='> 0 indica cauda longa à direita',
      )
      col_stat2.metric(
          '⛰️ Curtose de Despesas (Kurtosis)',
          f'{kurtosis:.2f}',
          delta='Achatamento da distribuição',
      )
      ...
      for idx, row in pivot_mensal.iterrows():
        ...
        val = row['Expense']
        z_score = (
            (val - media_geral) / desvio_padrao if desvio_padrao > 0 else 0.0
        )
      ...
      st.dataframe(
          aplicar_estilo_tabela(
              pd.DataFrame(dados_stats).set_index('Mês').style
          ),
          use_container_width=True,
      )
      ...
      st.info(
          'Requer pelo menos 3 meses registrados para calcular métricas'
          ' estatísticas avançadas (Skewness/Kurtosis).'
      )
  else:
    st.info('Nenhum dado de Budget cadastrado para calcular os indicadores.')
  ...
  descricao = st.text_input(
      'Descrição', placeholder='Ex: Gas, Supermercado, Pagamento de Fatura...'
  )

  lista_cat_opcao = st.session_state.categorias + [
  ...
  if tipo == 'Transferência':
    ...
    with col_t1:
      conta_saida_escolhida = st.selectbox(
          'Conta Saída (Origem)',
          contas_base + ['+ Incluir Novo Cartão/Conta...'],
      )
      ...
      if conta_saida_escolhida == '+ Incluir Novo Cartão/Conta...':
        ...
        if novo_c_digitado.strip() != '':
          ...
          st.session_state.cartoes = pd.concat(
              [st.session_state.cartoes, novo_c_df], ignore_index=True
          )
          st.session_state.cartoes.to_csv(ARQUIVO_CARTOES, index=False)
    with col_t2:
      conta_destino_escolhida = st.selectbox(
          'Conta Destino', contas_base + ['+ Incluir Novo Cartão/Conta...']
      )
      ...
      if conta_destino_escolhida == '+ Incluir Novo Cartão/Conta...':
        ...
        if novo_d_digitado.strip() != '':
          ...
          novo_d_df = pd.DataFrame([{
              'Nome': conta_destino_final,
              'Fechamento': 10,
              'Limite': 1000.0,
              'Vencimento': 17,
          }])
          st.session_state.cartoes = pd.concat(
              [st.session_state.cartoes, novo_d_df], ignore_index=True
          )
          st.session_state.cartoes.to_csv(ARQUIVO_CARTOES, index=False)
    ...
    if (
        not st.session_state.cartoes.empty
        and conta_final in st.session_state.cartoes['Nome'].values
    ):
      cartao_selecionado_row = st.session_state.cartoes[
          st.session_state.cartoes['Nome'] == conta_final
      ].iloc[0]
  ...
  parcelas = st.number_input(
      'Installments (Parcelas)', min_value=1, max_value=48, value=1
  )
  frequencia = st.selectbox(
      'Frequência', ['Mensal', 'Quinzenal', 'Anual', 'Única']
  )
  modo_valor = st.selectbox(
      'Modo de Valor', ['Dividir Total', 'Replicar Integral']
  )

  if st.button('Salvar Lançamento', type='primary'):
    if (
        cat_escolhida == '+ Incluir Nova Categoria...'
        and categoria_final not in st.session_state.categorias
    ):
      st.session_state.categorias.append(categoria_final)
      pd.DataFrame({'Categoria': st.session_state.categorias}).to_csv(
          ARQUIVO_CATEGORIAS, index=False
      )
    ...
    if descricao.strip() == '':
      ...
      eh_cartao = (
          tipo == 'Despesa'
          and cartao_selecionado_row is not None
          and 'Fechamento' in cartao_selecionado_row
      )
      dia_fechamento = (
          int(cartao_selecionado_row['Fechamento']) if eh_cartao else 0
      )
      dia_vencimento = (
          int(cartao_selecionado_row['Vencimento'])
          if (eh_cartao and 'Vencimento' in cartao_selecionado_row)
          else 10
      )
      ...
      if (
          eh_cartao
          and dia_fechamento > 0
          and data_compra.day > dia_fechamento
      ):
      ...
      if eh_cartao:
        ...
        for i in range(parcelas):
          ...
          if dia_fechamento > 0:
            ...
            try:
              data_fatura_parcela = data_fatura_parcela.replace(
                  day=min(dia_vencimento, 28)
              )
            except:
              pass
        ...
        for i in range(parcelas):
          ...
          novos_registros.append({
              ...
              'Conta': conta_final,
              'ContaDestino': (
                  conta_destino_final if tipo == 'Transferência' else ''
              ),
              'Valor': round(valor_parcela, 2),
      ...
      st.session_state.lancamentos = pd.concat(
          [st.session_state.lancamentos, df_novo], ignore_index=True
      )
      ...
      if tipo != 'Transferência' and cartao_selecionado_row is not None:
        ...
        idx_cartao = st.session_state.cartoes[
            st.session_state.cartoes['Nome'] == nome_c_alvo
        ].index
        if not idx_cartao.empty:
          limite_atual = float(
              st.session_state.cartoes.loc[idx_cartao[0], 'Limite']
          )
      ...
      if (
          tipo == 'Transferência'
          and not st.session_state.cartoes.empty
          and conta_destino_final in st.session_state.cartoes['Nome'].values
      ):
        idx_cartao_dest = st.session_state.cartoes[
            st.session_state.cartoes['Nome'] == conta_destino_final
        ].index
        if not idx_cartao_dest.empty:
          limite_atual = float(
              st.session_state.cartoes.loc[idx_cartao_dest[0], 'Limite']
          )
      ...
      st.success('Lançamento(s) gerado(s) com sucesso!')

elif aba == 'Lançamentos':
  st.subheader('Lista de Lançamentos & Smart Search (Gerenciamento)')
  st.markdown(
      'Utilize a ferramenta de **Smart Search** para filtrar transações'
      ' instantaneamente e gerenciar cada registro (Editar ou Deletar).'
  )
  ...
  if not df.empty:
    ...
    with col_s1:
      termo_busca = st.text_input(
          '🔍 Smart Search (Pesquisa Inteligente)',
          placeholder='Digite descrição, categoria ou conta...',
      )
    with col_s2:
      filtro_tipo = st.selectbox(
          'Filtrar por Tipo', ['Todos', 'Receita', 'Despesa', 'Transferência']
      )
    with col_s3:
      filtro_status = st.selectbox(
          'Filtrar por Status', ['Todos', 'Budget', 'Efetivado']
      )
    ...
    if not df_filtrado.empty:
      for idx, row in df_filtrado.iterrows():
        with st.expander(
            f"[{row['Tipo']}] {row['Data']} - {row['Descricao']} | R$"
            f" {row['Valor']:,.2f} ({row['Status']})"
        ):
          ...
          with col_btn1:
            if st.button(
                '🗑️ Deletar Lançamento', key=f'del_{idx}', type='secondary'
            ):
              st.session_state.lancamentos = (
                  st.session_state.lancamentos.drop(idx)
                  .reset_index(drop=True)
              )
              st.session_state.lancamentos.to_csv(
                  ARQUIVO_LANCAMENTOS, index=False
              )
              st.session_state.lancamentos = pd.read_csv(ARQUIVO_LANCAMENTOS)
          ...
          if st.session_state.get(f'editar_toggle_{idx}', False):
            with st.form(key=f'form_edit_{idx}'):
              ...
              idx_tipo = (
                  tipos_disp.index(row['Tipo'])
                  if row['Tipo'] in tipos_disp
                  else 0
              )
              novo_tipo = st.selectbox(
                  'Tipo', tipos_disp, index=idx_tipo, key=f'et_{idx}'
              )
              novo_status = st.selectbox(
                  'Status',
                  ['Budget', 'Efetivado'],
                  index=0 if row['Status'] == 'Budget' else 1,
                  key=f'es_{idx}',
              )
              nova_desc = st.text_input(
                  'Descrição', value=row['Descricao'], key=f'ed_{idx}'
              )

              idx_cat = (
                  st.session_state.categorias.index(row['Categoria'])
                  if row['Categoria'] in st.session_state.categorias
                  else 0
              )
              ...
              if not st.session_state.cartoes.empty:
                contas_possiveis.extend(
                    st.session_state.cartoes['Nome'].tolist()
                )
              idx_conta = (
                  contas_possiveis.index(row['Conta'])
                  if row['Conta'] in contas_possiveis
                  else 0
              )
              ...
              if novo_tipo == 'Transferência':
                idx_conta_dest = (
                    contas_possiveis.index(row.get('ContaDestino', ''))
                    if row.get('ContaDestino', '') in contas_possiveis
                    else 0
                )
                nova_conta_dest = st.selectbox(
                    'Conta Destino',
                    contas_possiveis,
                    index=idx_conta_dest,
                    key=f'econta_dest_{idx}',
                )
              ...
              try:
                data_parsed = datetime.strptime(
                    str(row['Data']).split()[0], '%Y-%m-%d'
                ).date()
              except:
                data_parsed = datetime.today().date()
              nova_data = st.date_input(
                  'Data', value=data_parsed, key=f'edata_{idx}'
              )
              ...
              if salvar_edicao:
                ...
                st.session_state.lancamentos.loc[idx, 'ContaDestino'] = (
                    nova_conta_dest if novo_tipo == 'Transferência' else ''
                )
                st.session_state.lancamentos.loc[idx, 'Valor'] = float(
                    novo_valor
                )
      ...
      st.warning(
          'Nenhum lançamento corresponde ao filtro ou Smart Search informado.'
      )
  ...
  with st.form('form_cartao'):
    nome_cartao = st.text_input(
        'Nome do Cartão / Banco', placeholder='Ex: Visa Itaú...'
    )
    dia_fechamento = st.number_input(
        'Dia de Fechamento da Fatura', min_value=1, max_value=31, value=10
    )
    limite_disponivel = st.number_input(
        'Limite Disponível (R$)', min_value=0.0, format='%.2f', value=1000.0
    )
    dia_pagamento = st.number_input(
        'Dia de Vencimento / Pagamento', min_value=1, max_value=31, value=17
    )
    ...
    if salvar_cartao:
      if nome_cartao.strip() != '':
        ...
        st.session_state.cartoes = pd.concat(
            [st.session_state.cartoes, novo_cartao], ignore_index=True
        )
        st.session_state.cartoes.to_csv(ARQUIVO_CARTOES, index=False)
  ...
  if not st.session_state.cartoes.empty:
    for idx, row in st.session_state.cartoes.iterrows():
      ...
      col1.write(
          f"**{row['Nome']}** | Fechamento: dia {row['Fechamento']} | Vencimento:"
          f" dia {row['Vencimento']} | Limite: R$ {row['Limite']:,.2f}"
      )

      tem_lancamento = False
      if (
          not st.session_state.lancamentos.empty
          and 'Conta' in st.session_state.lancamentos.columns
      ):
        tem_lancamento = (
            st.session_state.lancamentos['Conta'] == row['Nome']
        ).any()

      if col2.button('🗑️ Excluir', key=f'del_cartao_{idx}'):
        if tem_lancamento:
          st.error(
              f"Não é possível excluir '{row['Nome']}': existem lançamentos"
              ' vinculados a esta conta.'
          )
        else:
          st.session_state.cartoes = (
              st.session_state.cartoes.drop(idx).reset_index(drop=True)
          )
          st.session_state.cartoes.to_csv(ARQUIVO_CARTOES, index=False)
  ...
  st.subheader('📊 KPIs Avançados - FP&A & Corporate Finance')
  st.markdown(
      'Indicadores de alto desempenho calculados com base no fluxo operacional e'
      ' alavancagem.'
  )
  ...
  if not df.empty:
    ...
    # Coleta de dados globais para simulação dos 5 KPIs FPA
    receita_total = df[df['Tipo'] == 'Receita']['Valor'].sum()
    despesa_total = df[df['Tipo'] == 'Despesa']['Valor'].sum()
    caixa_liquido = max(0.0, receita_total - despesa_total)
    ...
    total_passivos = df[
        ...
        & (
            df['Conta'].isin(cartoes_nomes)
            | df['Categoria'].str.contains(
                'debt|dívida|cartão', case=False, na=False
            )
        )
    ]['Valor'].sum()
    ...
    # 1. Margem OCF
    if receita_total > 0:
      ocf_margin = ((receita_total - despesa_total) / receita_total) * 100
    else:
      ocf_margin = 0.0

    # 2. DSR (Gastos Discricionários)
    despesas_df = df[df['Tipo'] == 'Despesa']
    discricionarias = despesas_df[
        despesas_df['subtipo']
        .astype(str)
        .str.contains('Discricionário|Lazer|Flexível', case=False, na=False)
    ]['Valor'].sum()
    dsr = (
        (discricionarias / despesa_total) * 100 if despesa_total > 0 else 0.0
    )

    # 3. FCCR (Cobertura Fixa)
    custos_fixos = despesas_df[
        despesas_df['subtipo']
        .astype(str)
        .str.contains('Fixo|Obrigatório|Essencial', case=False, na=False)
    ]['Valor'].sum()
    if custos_fixos > 0:
      fccr = receita_total / custos_fixos
    else:
      fccr = float('inf') if receita_total > 0 else 0.0
    ...
    rule_of_wealth = poupanca_pct  # Simplificado para o agregado atual

    # 5. Net Leverage Ratio
    if caixa_liquido > 0:
      net_leverage = total_passivos / caixa_liquido
    else:
      net_leverage = float('inf') if total_passivos > 0 else 0.0
    ...
    with col1:
      st.metric(
          label='Margem OCF',
          value=f'{ocf_margin:.2f}%',
          delta='Ideal > 30%',
          delta_color='normal',
      )
    with col2:
      st.metric(
          label='Gastos Discricionários (DSR)',
          value=f'{dsr:.2f}%',
          delta='Flexibilidade Orçamentária',
          delta_color='off',
      )
    with col3:
      fccr_str = f'{fccr:.2f}x' if fccr != float('inf') else '∞'
      st.metric(
          label='Cobertura Fixa (FCCR)',
          value=fccr_str,
          delta='Meta > 1.2x',
          delta_color='normal' if fccr >= 1.2 else 'inverse',
      )

    col4, col5 = st.columns(2)
    with col4:
      st.metric(
          label='Rule of Wealth',
          value=f'{rule_of_wealth:.2f}%',
          delta='Meta > 20%',
          delta_color='normal' if rule_of_wealth >= 20 else 'inverse',
      )
    with col5:
      net_lev_str = (
          f'{net_leverage:.2f}x' if net_leverage != float('inf') else '∞'
      )
      st.metric(
          label='Net Leverage Ratio',
          value=net_lev_str,
          delta='Menor é melhor (< 0.5x)',
          delta_color='inverse' if net_leverage > 0.5 else 'normal',
      )
  else:
  ...
  if st.button('Adicionar Categoria'):
    if (
        nova_cat.strip() != ''
        and nova_cat not in st.session_state.categorias
    ):
