...
# Inicializar estados na memória
...
if aba == "Dashboard":
    ...
    contas_disponiveis = ["Cash husband", "Nubank"]
    if not st.session_state.cartoes.empty:
        contas_disponiveis.extend(st.session_state.cartoes["Nome"].tolist())
    ...
    with st.form("form_cadastro"):
        ...
        # Opção de categoria e inclusão rápida
        cat_opcao = st.selectbox("Categoria", st.session_state.categorias + ["+ Incluir Nova Categoria..."])
        
        nova_cat_input = ""
        if cat_opcao == "+ Incluir Nova Categoria...":
            nova_cat_input = st.text_input("Digite o nome da nova categoria:")

        conta = st.selectbox("Conta / Cartão", contas_disponiveis)
        ...
        if enviado:
            ...
            if cat_opcao == "+ Incluir Nova Categoria...":
                if nova_cat_input.strip() != "":
                    categoria_final = nova_cat_input.strip()
    ...
    with st.form("form_cartao"):
        nome_cartao = st.text_input("Nome do Cartão / Banco", placeholder="Ex: Visa Itaú, Mastercard...")
    ...
    with st.form("form_nova_categoria"):
        nova_cat = st.text_input("Nome da Nova Categoria", placeholder="Ex: Educação, Saúde...")
        ...
        if adicionar_cat:
            ...
            elif nova_cat in st.session_state.categorias:
